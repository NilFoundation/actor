...
def coverageReport() {
    dir('caf-sources') {
        sh 'gcovr -e examples -e tools -e test -e ".*/test/.*" -e core/caf/scheduler/profiled_coordinator.hpp -x -r . > coverage.xml'
    ...
    stages {
        stage('Git Checkout') {
            ...
            steps {
                ...
                dir('caf-sources') {
                    checkout scm
                    sh './scripts/get-detach-version.sh'
                }
                    ...
                    sh '''
                        if [ "$(cat ../caf-sources/branch.txt)" = "master" ]; then
                            ...
                            if [ -n "$(git status --porcelain)" ]; then
                                ...
                                if [ -z "$(grep 'exp.sha' ../caf-sources/detach.txt)" ] ; then
                                    git tag $(cat ../caf-sources/detach.txt)
                                    git push origin $(cat ../caf-sources/detach.txt)
                                fi
                            fi
                        fi
                    '''
