...
public:
    ...
    host_desc(std::string host_addr, int slots, string cldevices) : host(std::move(host_addr)), cpu_slots(slots),
    ...
    static void append(vector<host_desc> &xs, const string &line, size_t num) {
        ...
        for (auto i = fields.begin() + 1; i != fields.end(); ++i) {
            if (starts_with(*i, "device_ids=")) {
                hd.opencl_device_ids.assign(std::find(i->begin(), i->end(), '=') + 1, i->end());
...
bool run_ssh(actor_system &system, const string &wdir, const string &cmd, const string &host) {
    ...
    oss << "ssh -Y -o ServerAliveInterval=60 " << host << R"( "echo )" << packed << R"( | base64 --decode | /bin/sh")";
...
void bootstrap(actor_system &system, const string &wdir, const host_desc &master, vector<host_desc> slaves,
    ...
    if (!port_res) {
        cerr << "fatal: unable to publish actor: " << system.render(port_res.error()) << endl;
        return;
    }
    auto port = *port_res;
    ...
    for (size_t i = 0; i < slaves.size(); ++i) {
        self->receive([&](const string &host, uint16_t slave_port) {
            if (!slaveslist.empty()) {
                slaveslist += ',';
            }
            slaveslist += host;
