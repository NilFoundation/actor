...
std::istream &skip_word(std::istream &in) {
    skip_whitespaces(in);
    auto nonspace = [](char x) {
        return (isprint(x) != 0) && (isspace(x) == 0);
    };
...
constexpr const char *log_level_name[] = {"ERROR", "WARN", "INFO", "DEBUG", "TRACE", "?????"};

enum class log_level {
    error, warn, info, debug, trace, invalid
};
...
typename Inspector::result_type inspect(Inspector &f, entity &x) {
    return f(meta::type_name("entity"), x.aid, x.tid, x.nid, x.vid, x.hidden, x.pretty_name);
}
...
/// MTL events according to SE-0001.
enum class se_type {
    spawn, init, send, reject, receive, drop, skip, finalize, terminate, none
};

string to_string(se_type x) {
    const char *tbl[] = {"spawn", "init", "send", "reject", "receive", "drop", "skip", "finalize", "terminate", "none"};
...
std::ostream &operator<<(std::ostream &out, const enhanced_log_entry &x) {
    return out << x.json_vstamp << ' ' << x.data.timestamp << ' ' << x.data.component << ' ' << x.data.level << ' '
               << x.id.pretty_name << ' ' << x.data.class_name << ' ' << x.data.function_name << ' ' << x.data.file_name
               << ':' << x.data.line_number << ' ' << x.data.message;
}

std::istream &operator>>(std::istream &in, log_entry &x) {
    in >> x.timestamp >> x.component >> x.level >> consume("actor") >> x.id.aid >> x.id.tid >> x.class_name
       >> x.function_name >> skip_whitespaces >> rd_line(x.file_name, ':') >> x.line_number >> skip_whitespaces
       >> rd_line(x.message);
...
expected <first_pass_result> first_pass(blocking_actor *self, std::istream &in, verbosity_level vl) {
    ...
    logger_id id;
    string message;
    while (in >> skip_word >> skip_word >> skip_word >> id >> skip_word >> skip_word >> skip_word >> rd_line(message)) {
        // store in map
    ...
    if (vl >= verbosity_level::informative) {
        aout(self) << "found " << res.entities.size() << " entities for node " << res.this_node << std::endl;
    }
    return res;
}
...
void second_pass(blocking_actor *self, const group &grp, const entity_set &entities, const node_id &nid,
                 const std::vector<string> &json_names, std::istream &in, std::ostream &out, std::mutex &out_mtx,
    ...
    for (auto &x : local_entities) {
        ...
        local_entities_state.emplace(logger_id{x.aid, x.tid}, state_t{x, std::move(vzero)});
    }
    ...
    auto bcast = [&](const se_event &x) {
        if (vl >= verbosity_level::noisy) {
            aout(self) << "broadcast event from " << nid << ": " << deep_to_string(x) << std::endl;
        }
        if (self != nullptr) {
            self->send(grp, x);
        }
    ...
    auto fetch_message = [&](const std::map<string, string> &fields) -> se_event & {
        // TODO: this receive unconditionally waits on a message,
        //       i.e., is a potential deadlock
        if (vl >= verbosity_level::noisy) {
            aout(self) << "wait for send from another node matching fields " << deep_to_string(fields) << std::endl;
        }
    ...
    while (in >> plain_entry) {
            ...
            switch (event.type) {
                ...
                case se_type::init: {
                    ...
                    if (i != e) {
                        ...
                        std::cerr << "*** cannot match init event to a previous spawn" << std::endl;
                    }
                    break;
                }
        ...
        if (!internal) {
            boost::lock_guard<std::mutex> guard{out_mtx};
            out << entry << '\n';
        }
    }
}
...
namespace {
    ...
    struct config : public actor_system_config {
        ...
        config() {
            opt_group{custom_options_, "global"}.add(output_file, "output-file,o", "Path for the output file").add(
                    include_hidden_actors, "include-hidden-actors,i", "Include hidden (system-level) actors").add(
                    verbosity, "verbosity,v", "Debug output (from 0 to 2)");
...
// clock timestamps
    void caf_main(actor_system &sys, const config &cfg) {
        ...
        static constexpr size_t irsize = sizeof(file_path) + sizeof(std::ifstream) + sizeof(first_pass_result);
        ...
        struct intermediate_res {
            ...
            intermediate_res(file_path fp, ifstream_ptr fs, first_pass_result &&fr) : fname(std::move(fp)),
                    fstream(std::move(fs)), res(std::move(fr)) {
                // nop
            }
        };
        ...
        std::sort(intermediate_results.begin(), intermediate_results.end(), sort_pred);
        for (auto &ir : intermediate_results) {
            ...
            for (auto &kvp : ir.res.entities) {
                ...
                entities.emplace(entity{kvp.first.aid, kvp.first.tid, ir.res.this_node, vid, kvp.second.hidden,
        ...
        for (auto &fpr : intermediate_results) {
            sys.spawn_in_group(grp, [&](blocking_actor *self) {
                second_pass(self, grp, entities, fpr.res.this_node, entity_names, *fpr.fstream, out, out_mtx,
                            !cfg.include_hidden_actors, vl);
            });
        }
        sys.await_all_actors_done();
    }
