...
Message passing in ACTOR is always asynchronous. Further, ACTOR neither guarantees
message delivery nor message ordering in a distributed setting. ACTOR uses TCP
...
The messaging layer of ACTOR has three primitives for sending messages:
...
When enqueuing a message to the mailbox of an actor, ACTOR wraps the content of
...
allows ACTOR to build pipelines of arbitrary size. If no more stage is left, the
response reaches the sender. Finally, \lstinline^content()^ grants access to
the type-erased tuple storing the message itself.

Mailbox elements are created by ACTOR automatically and are usually invisible to
...
It is worth mentioning that ACTOR usually wraps the mailbox element and its
...
ACTOR allows multiple actors to implicitly share message contents, as long as no
...
Message types in ACTOR must meet the following requirements:
...
Requirement 2 is a consequence of requirement 1, because ACTOR needs to be able
to create an object of a type before it can call \lstinline^serialize^ or
\lstinline^inspect^ on it. Requirement 3 allows ACTOR to implement Copy on
Write~\see{copy-on-write}.

\subsection{Default and System Message Handlers}
\label{special-handler}

ACTOR has three system-level message types (\lstinline^down_msg^,
...
an actor to handle it in a later state. ACTOR provides the following built-in
...
A main feature of ACTOR is its ability to couple input and output types via the
...
input and responds with another \lstinline^int^. ACTOR embraces this functional
take on actors by simply creating response messages from the result of message
handlers. This allows ACTOR to match \emph{request} to \emph{response} messages
...
Requests allow ACTOR to unambiguously correlate request and response messages.
This is also true if the response is an error message. Hence, ACTOR allows to
