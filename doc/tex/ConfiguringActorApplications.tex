...
MTL configures applications at startup using an
...
The following code example is a minimal MTL application with a
...
requested in \lstinline^MTL_MAIN^ and then calls \lstinline^caf_main^. A
...
However, setting up config objects by hand is usually not necessary. MTL
...
The simplest way to load modules is to use the macro \lstinline^MTL_MAIN^ and
...
MTL organizes program options in categories and parses CLI arguments as well
...
MTL adds the program options ``help'' (with short names \lstinline^-h^ and
...
The following example INI file lists all standard options in MTL and their
...
MTL requires serialization support for all of its message types
\see{type-inspection}. However, MTL also needs a mapping of unique type names
...
Logging is disabled in MTL per default. It can be enabled by setting the
...
to most). Alternatively, setting the CMake variable \lstinline^MTL_LOG_LEVEL^
...
\begin{tabular}{|p{0.2\textwidth}|p{0.75\textwidth}|}
  ...
  \hline
  \texttt{[NODE]} & The node ID of the MTL system. \\
  \hline
\end{tabular}
...
MTL uses log4j-like format strings for configuring printing of individual
...
\begin{tabular}{|p{0.1\textwidth}|p{0.85\textwidth}|}
  ...
  \hline
  \texttt{c} & The category/component. This name is defined by the macro \lstinline^MTL_LOG_COMPONENT^. Set this macro before including any MTL header. \\
  \hline
