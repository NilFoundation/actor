...
ACTOR configures applications at startup using an
\lstinline^spawner_config^ or a user-defined subclass of that type. The
...
The following code example is a minimal ACTOR application with a
middleman~\see{middleman} but without any custom configuration options.

\begin{lstlisting}
void caf_main(spawner& system) {
  // ...
}
ACTOR_MAIN(io::middleman)
\end{lstlisting}
...
requested in \lstinline^ACTOR_MAIN^ and then calls \lstinline^caf_main^. A
...
However, setting up config objects by hand is usually not necessary. ACTOR
automatically selects user-defined subclasses of
\lstinline^spawner_config^ if \lstinline^caf_main^ takes a second
parameter by reference, as shown in the minimal example below.

\begin{lstlisting}
class my_config : public spawner_config {
public:
  my_config() {
    // ...
  }
...
void caf_main(spawner& system, const my_config& cfg) {
  // ...
}
...
The simplest way to load modules is to use the macro \lstinline^ACTOR_MAIN^ and
...
ACTOR organizes program options in categories and parses CLI arguments as well
...
implementing a subtype of \lstinline^spawner_config^. The example below
...
ACTOR adds the program options ``help'' (with short names \lstinline^-h^ and
...
The following example INI file lists all standard options in ACTOR and their
...
ACTOR requires serialization support for all of its message types
\see{type-inspection}. However, ACTOR also needs a mapping of unique type names
...
Logging is disabled in ACTOR per default. It can be enabled by setting the
...
to most). Alternatively, setting the CMake variable \lstinline^ACTOR_LOG_LEVEL^
...
\begin{tabular}{|p{0.2\textwidth}|p{0.75\textwidth}|}
  ...
  \hline
  \texttt{[NODE]} & The node ID of the ACTOR system. \\
  \hline
\end{tabular}
...
ACTOR uses log4j-like format strings for configuring printing of individual
...
\begin{tabular}{|p{0.1\textwidth}|p{0.85\textwidth}|}
  ...
  \hline
  \texttt{c} & The category/component. This name is defined by the macro \lstinline^ACTOR_LOG_COMPONENT^. Set this macro before including any ACTOR header. \\
  \hline
