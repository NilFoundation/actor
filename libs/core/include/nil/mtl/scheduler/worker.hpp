    ...
    namespace mtl {
        namespace scheduler {
            ...
            public:
                ...
                void start() {
                    MTL_ASSERT(this_thread_.get_id() == std::thread::id {});
                    auto this_worker = this;
                    this_thread_ = std::thread {[this_worker] {
                        MTL_SET_LOGGER_SYS(&this_worker->system());
            ...
            private:
                void run() {
                    ...
                    for (;;) {
                        ...
                        MTL_ASSERT(job != nullptr);
                        MTL_ASSERT(job->subtype() != resumable::io_actor);
                        MTL_PUSH_AID_FROM_PTR(dynamic_cast<abstract_actor *>(job));
