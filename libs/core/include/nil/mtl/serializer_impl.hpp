...
namespace nil {
    namespace mtl {
        ...
        public:
            ...
            static_assert(sizeof(value_type) == 1, "container must have a byte-sized value type");

            // -- constructors, destructors, and assignment operators --------------------

            serializer_impl(actor_system &sys, container_type &buf) : super(sys), buf_(buf), write_pos_(buf_.size()) {
                // nop
            }

            serializer_impl(execution_unit *ctx, container_type &buf) : super(ctx), buf_(buf), write_pos_(buf_.size()) {
                // nop
            }
            ...
            error apply_raw(size_t num_bytes, void *data) override {
                ...
                if (write_pos_ == buf_size) {
                    ...
                    MTL_ASSERT(remaining < num_bytes);
        ...
        protected:
            ...
            error apply_impl(std::string &x) override {
                ...
                if (str_size == 0)
                    return error::eval([&] { return begin_sequence(str_size); }, [&] { return end_sequence(); });
