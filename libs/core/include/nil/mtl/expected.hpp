...
namespace nil {
    namespace mtl {
        ...
        public:
            ...
            expected(nil::mtl::error e) noexcept : engaged_(false) {
                new (std::addressof(error_)) nil::mtl::error {std::move(e)};
            }

            expected(no_error_t) noexcept : engaged_(false) {
                new (std::addressof(error_)) nil::mtl::error {};
            }
            ...
            expected(Code code) : engaged_(false) {
                new (std::addressof(error_)) nil::mtl::error(make_error(code));
            }
            ...
            expected &operator=(nil::mtl::error e) noexcept {
                if (!engaged_)
                    error_ = std::move(e);
                else {
                    destroy();
            ...
            /// @pre `engaged() == false`.
            const nil::mtl::error &cerror() const noexcept {
                MTL_ASSERT(!engaged_);
                return error_;
            }

            /// @copydoc cerror
            const nil::mtl::error &error() const noexcept {
                MTL_ASSERT(!engaged_);
                return error_;
            }
        ...
        private:
            void construct(expected &&other) noexcept(nothrow_move) {
                if (other.engaged_)
                    ...
                    new (std::addressof(error_)) nil::mtl::error(std::move(other.error_));
            ...
            void construct(const expected &other) noexcept(nothrow_copy) {
                if (other.engaged_)
                    ...
                    new (std::addressof(error_)) nil::mtl::error(other.error_);
                engaged_ = other.engaged_;
            }
        ...
        public:
            ...
            expected(nil::mtl::error e) noexcept : error_(std::move(e)) {
                // nop
            }
            ...
            const nil::mtl::error &error() const {
                return error_;
            }
...
namespace std {
    ...
    template<class T>
    auto operator<<(ostream &oss, const nil::mtl::expected<T> &x) -> decltype(oss << *x) {
        if (x)
            oss << *x;
        else
