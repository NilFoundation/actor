...
namespace nil {
    namespace mtl {
        ...
        /// A node ID is an opaque value for representing MTL instances in the network.
        class node_id {
        public:
            // -- member types -----------------------------------------------------------
        ...
        /// @param host_id Unique hash value representing a single MTL node.
        /// @relates node_id
...
namespace std {
    ...
    template<>
    struct hash<nil::mtl::node_id> {
        size_t operator()(const nil::mtl::node_id &x) const noexcept {
