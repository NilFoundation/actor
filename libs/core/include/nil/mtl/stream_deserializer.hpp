...
namespace nil {
    namespace mtl {
        ...
        public:
            ...
            explicit stream_deserializer(S &&sb) : deserializer(nullptr), streambuf_(std::forward<S>(sb)) {
            }
            ...
            error apply_raw(size_t num_bytes, void *data) override {
                return range_check(
                    streambuf_.sgetn(reinterpret_cast<char_type *>(data), static_cast<streamsize>(num_bytes)),
                    num_bytes);
            }
        ...
        protected:
            ...
            error range_check(std::streamsize got, size_t need) {
                if (got >= 0 && static_cast<size_t>(got) == need)
                    return none;
                MTL_LOG_ERROR("range_check failed");
                return sec::end_of_stream;
            }
