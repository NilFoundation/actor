...
#define MTL_READ_INSPECTOR_TRY(statement)                                           \
    using MTL_UNIFYN(statement_result) = decltype(statement);                       \
    if constexpr (std::is_same<MTL_UNIFYN(statement_result), void>::value) {        \
        statement;                                                                  \
    } else if constexpr (std::is_same<MTL_UNIFYN(statement_result), bool>::value) { \
        if (!statement)                                                             \
            return false;                                                           \
        ...
        private:
            ...
            typename std::enable_if<!meta::is_annotation_v<T>, bool>::type try_apply(R &result, const T &x) {
                ...
                if constexpr (std::is_empty<T>::value || is_allowed_unsafe_message_type_v<T>) {
                    ...
                    MTL_READ_INSPECTOR_TRY(dref.apply(squashed_x));
                } else if constexpr (detail::can_apply<Subtype, decltype(x)>::value) {
                    MTL_READ_INSPECTOR_TRY(dref.apply(x));
                } else if constexpr (std::is_array<T>::value) {
                    std::make_index_sequence<std::extent<T>::value> seq;
                    MTL_READ_INSPECTOR_TRY(apply_array(dref, x, seq));
                } else if constexpr (detail::is_stl_tuple_type<T>::value) {
                    std::make_index_sequence<std::tuple_size<T>::value> seq;
                    MTL_READ_INSPECTOR_TRY(apply_tuple(dref, x, seq));
                } else if constexpr (detail::is_map_like<T>::value) {
                    MTL_READ_INSPECTOR_TRY(dref.begin_sequence(x.size()));
                    for (const auto &kvp : x) {
                        MTL_READ_INSPECTOR_TRY(dref(kvp.first, kvp.second));
                    }
                    MTL_READ_INSPECTOR_TRY(dref.end_sequence());
                } else if constexpr (detail::is_list_like<T>::value) {
                    MTL_READ_INSPECTOR_TRY(dref.begin_sequence(x.size()));
                    for (const auto &value : x) {
                        MTL_READ_INSPECTOR_TRY(dref(value));
                    }
                    MTL_READ_INSPECTOR_TRY(dref.end_sequence());
                } else {
                    ...
                    MTL_READ_INSPECTOR_TRY(inspect(dref, mutable_x));
                }
                return true;
            }
        };
