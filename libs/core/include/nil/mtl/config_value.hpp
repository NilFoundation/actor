...
#include <nil/mtl/detail/move_if_not_ptr.hpp>
...
namespace nil {
    namespace mtl {
        ...
        public:
            ...
            using timespan = nil::mtl::timespan;

            using string = std::string;

            using list = std::vector<config_value>;

            using dictionary = nil::mtl::dictionary<config_value>;
        ...
        struct select_config_value_access<T, true> {
            struct type {
                static bool is(const config_value &x) {
                    auto ptr = nil::mtl::get_if<typename config_value::integer>(x.get_data_ptr());
        ...
        struct config_value_access<float> {
            ...
            static optional<float> get_if(const config_value *x) {
                auto res = nil::mtl::get_if<double>(&(x->get_data()));
                if (res)
            ...
            static float get(const config_value &x) {
                return static_cast<float>(nil::mtl::get<double>(x.get_data()));
            }
        };
        ...
        struct config_value_access<std::vector<T>> {
            ...
            static bool is(const config_value &x) {
                auto lst = nil::mtl::get_if<config_value::list>(&x);
                if (lst != nullptr) {
                    return std::all_of(lst->begin(), lst->end(),
                                       [](const config_value &y) { return nil::mtl::holds_alternative<T>(y); });
                }
                return false;
            }
        ...
        struct config_value_access<std::tuple<Ts...>> {
            ...
            static bool is(const config_value &x) {
                if (auto lst = nil::mtl::get_if<config_value::list>(&x)) {
        ...
        private:
            ...
            static bool rec_get(const config_value::list &xs, tuple_type &result, detail::int_token<Pos>,
                ...
                if (auto value = nil::mtl::get_if<U>(&xs[Pos])) {
        ...
        struct config_value_access<dictionary<V>> {
            ...
            static bool is(const config_value &x) {
                auto lst = nil::mtl::get_if<config_value::dictionary>(&x);
                if (lst != nullptr) {
                    return std::all_of(lst->begin(), lst->end(),
            ...
            static optional<map_type> get_if(const config_value *x) {
                ...
                auto extract = [&](const kvp &y) {
                    auto opt = nil::mtl::get_if<V>(&(y.second));
                    if (opt) {
