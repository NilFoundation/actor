...
#define MTL_TYPE_NAME_BUILDER_NOINT(class_name, pretty_name) \
    template<>                                               \
    struct type_name_builder<class_name, false> {            \
        void operator()(std::string &result) const {         \
            ...
            MTL_TYPE_NAME_BUILDER_NOINT(float, "32-bit real");

            MTL_TYPE_NAME_BUILDER_NOINT(double, "64-bit real");

            MTL_TYPE_NAME_BUILDER_NOINT(timespan, "timespan");

            MTL_TYPE_NAME_BUILDER_NOINT(std::string, "string");

            MTL_TYPE_NAME_BUILDER_NOINT(atom_value, "atom");

            MTL_TYPE_NAME_BUILDER_NOINT(uri, "uri");

#undef MTL_TYPE_NAME_BUILDER
