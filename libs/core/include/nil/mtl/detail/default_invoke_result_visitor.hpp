        ...
        namespace detail {
            ...
            private:
                void deliver(response_promise &rp, error &x) {
                    MTL_LOG_DEBUG("report error back to requesting actor");
                    rp.deliver(std::move(x));
                }

                void deliver(response_promise &rp, message &x) {
                    MTL_LOG_DEBUG("respond via response_promise");
                ...
                void delegate(T &x) {
                    ...
                    if (!rp.pending()) {
                        MTL_LOG_DEBUG("suppress response message: invalid response promise");
                        return;
                    }
                    deliver(rp, x);
                }
