...
#include <nil/mtl/weak_intrusive_ptr.hpp>
...
namespace nil {
    namespace mtl {
        ...
        ///
        /// When allocating a new actor, MTL will always embed the user-defined
        ...
        public:
            ...
            inline abstract_actor *get() {
                ...
                return reinterpret_cast<abstract_actor *>(reinterpret_cast<intptr_t>(this) + MTL_CACHE_LINE_SIZE);
            }
            ...
            static actor_control_block *from(const abstract_actor *ptr) {
                // this pointer arithmetic is compile-time checked in actor_storage's ctor
                return reinterpret_cast<actor_control_block *>(reinterpret_cast<intptr_t>(ptr) - MTL_CACHE_LINE_SIZE);
            }
...
namespace std {
    ...
    template<>
    struct hash<nil::mtl::strong_actor_ptr> {
        inline size_t operator()(const nil::mtl::strong_actor_ptr &ptr) const {
            return ptr ? static_cast<size_t>(ptr->id()) : 0;
        }
    };

    template<>
    struct hash<nil::mtl::weak_actor_ptr> {
        inline size_t operator()(const nil::mtl::weak_actor_ptr &ptr) const {
            return ptr ? static_cast<size_t>(ptr->id()) : 0;
        }
    };
