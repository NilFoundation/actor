...
#include <nil/mtl/detail/algorithms.hpp>
#include <nil/mtl/detail/path_state.hpp>
#include <nil/mtl/detail/select_all.hpp>
    ...
    namespace mtl {
        ...
        public:
            ...
            void set_filter(stream_slot slot, filter_type new_filter) {
                MTL_LOG_TRACE(MTL_ARG(slot) << MTL_ARG(new_filter));
            ...
            bool insert_path(unique_path_ptr ptr) override {
                ...
                if (!super::insert_path(std::move(ptr))) {
                    MTL_LOG_DEBUG("unable to insert path at slot" << slot);
                    return false;
                }
                // Append to the state map.
                if (!state_map_.emplace(slot, path_state {}).second) {
                    MTL_LOG_DEBUG("unable to add state for slot" << slot);
            ...
            void emit_batches() override {
                MTL_LOG_TRACE(MTL_ARG2("buffered", this->buffered()) << MTL_ARG2("paths", this->paths_.size()));
        ...
        protected:
            void about_to_erase(outbound_path *ptr, bool silent, error *reason) override {
                ...
                MTL_LOG_TRACE(MTL_ARG2("slot", ptr->slots.sender) << MTL_ARG(silent) << MTL_ARG(reason));
