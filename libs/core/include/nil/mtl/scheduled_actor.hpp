...
#include <nil/mtl/default_downstream_manager.hpp>
#include <nil/mtl/error.hpp>
#include <nil/mtl/extend.hpp>
#include <nil/mtl/fwd.hpp>
#include <nil/mtl/inbound_path.hpp>
#include <nil/mtl/invoke_message_result.hpp>
#include <nil/mtl/is_actor_handle.hpp>
#include <nil/mtl/local_actor.hpp>
#include <nil/mtl/logger.hpp>
#include <nil/mtl/make_sink_result.hpp>
#include <nil/mtl/make_source_result.hpp>
#include <nil/mtl/make_stage_result.hpp>
...
#include <nil/mtl/stream_sink_trait.hpp>
...
#include <nil/mtl/detail/stream_sink_driver_impl.hpp>
#include <nil/mtl/detail/stream_sink_impl.hpp>
#include <nil/mtl/detail/stream_source_driver_impl.hpp>
#include <nil/mtl/detail/stream_source_impl.hpp>
#include <nil/mtl/detail/stream_stage_driver_impl.hpp>
#include <nil/mtl/detail/stream_stage_driver_impl.hpp>
...
#include <nil/mtl/intrusive/drr_cached_queue.hpp>
    ...
    namespace mtl {
        ...
        public:
            ...
            make_stage_result_t<In, DownstreamManager, Ts...> make_stage(const stream<In> &in, std::tuple<Ts...> xs,
                                                                         ...
                                                                         policy::arg<DownstreamManager> token = {}) {
                MTL_IGNORE_UNUSED(token);
                MTL_ASSERT(current_mailbox_element() != nullptr);
                MTL_ASSERT(current_mailbox_element()->content().match_elements<open_stream_msg>());
            ...
            void handle_upstream_msg(stream_slots slots, actor_addr &sender, T &x) {
                ...
                if (i == stream_managers_.end()) {
                    ...
                    MTL_LOG_INFO("no manager found:" << MTL_ARG(slots));
                ...
                MTL_ASSERT(i->second != nullptr);
                auto ptr = i->second;
                ptr->handle(slots, x);
                if (ptr->done()) {
                    MTL_LOG_DEBUG("done sending:" << MTL_ARG(slots));
                    ptr->stop();
                    erase_stream_manager(ptr);
                } else if (ptr->out().path(slots.receiver) == nullptr) {
                    MTL_LOG_DEBUG("done sending on path:" << MTL_ARG(slots.receiver));
