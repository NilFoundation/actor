...
bool cmp_one(const nil::mtl::message &x, const T &y) {
...
typename std::enable_if<(I == sizeof...(Ts)), bool>::type msg_cmp_rec(const nil::mtl::message &,
                                                                      const std::tuple<Ts...> &) {
    return true;
}

template<size_t I, class... Ts>
typename std::enable_if<(I < sizeof...(Ts)), bool>::type msg_cmp_rec(const nil::mtl::message &x,
...
private:
    ...
    typename std::enable_if<nil::mtl::meta::is_annotation<T>::value, bool>::type iterate(pos<X> pos, const T &,
                                                                                         const Ts &... ys) {
        return iterate(pos, ys...);
    }

    template<size_t X, class T, class... Ts>
    typename std::enable_if<!nil::mtl::meta::is_annotation<T>::value, bool>::type iterate(pos<X>, const T &y,
                                                                                          const Ts &... ys) {
...
// -- unified access to all actor handles in MTL -------------------------------

/// Reduces any of MTL's handle types to an `abstract_actor` pointer.
class mtl_handle : nil::mtl::detail::comparable<mtl_handle>, nil::mtl::detail::comparable<mtl_handle, std::nullptr_t> {
public:
    using pointer = nil::mtl::abstract_actor *;
    ...
    inline mtl_handle &operator=(nil::mtl::abstract_actor *x) {
        ptr_ = x;
        return *this;
    }
...
inline nil::mtl::mailbox_element *next_mailbox_element(mtl_handle x) {
    MTL_ASSERT(x.get() != nullptr);
    auto sptr = dynamic_cast<nil::mtl::scheduled_actor *>(x.get());
    if (sptr != nullptr) {
        return sptr->mailbox().closed() || sptr->mailbox().blocked() ? nullptr : sptr->mailbox().peek();
    }
    auto bptr = dynamic_cast<nil::mtl::blocking_actor *>(x.get());
    MTL_ASSERT(bptr != nullptr);
...
/// @private
template<class... Ts>
nil::mtl::optional<std::tuple<Ts...>> default_extract(mtl_handle x) {
...
/// @private
template<class T>
nil::mtl::optional<std::tuple<T>> unboxing_extract(mtl_handle x) {
    auto tup = default_extract<typename T::outer_type>(x);
    if (tup == nil::mtl::none || !is<T>(get<0>(*tup))) {
        return nil::mtl::none;
    }
...
struct try_extract_impl {
    nil::mtl::optional<std::tuple<T, Ts...>> operator()(mtl_handle x) {
...
struct try_extract_impl<T, true> {
    nil::mtl::optional<std::tuple<T>> operator()(mtl_handle x) {
...
nil::mtl::optional<std::tuple<T, Ts...>> try_extract(mtl_handle x) {
...
bool received(mtl_handle x) {
    return try_extract<T, Ts...>(x) != nil::mtl::none;
}

template<class... Ts>
class expect_clause {
public:
    expect_clause(nil::mtl::scheduler::test_coordinator &sched) : sched_(sched), dest_(nullptr) {
        peek_ = [=] {
    ...
    expect_clause &from(const Handle &whom) {
        src_ = nil::mtl::actor_cast<nil::mtl::strong_actor_ptr>(whom);
        return *this;
    }
    ...
    expect_clause &to(const Handle &whom) {
        ...
        dest_ = &sched_.next_job<nil::mtl::scheduled_actor>();
    ...
    expect_clause &to(const nil::mtl::scoped_actor &whom) {
        dest_ = whom.ptr();
        return *this;
    }
...
protected:
    void run_once() {
        auto dptr = dynamic_cast<nil::mtl::blocking_actor *>(dest_);
        if (dptr == nullptr) {
            sched_.run_once();
        } else {
            dptr->dequeue();
    ...
    nil::mtl::scheduler::test_coordinator &sched_;
    nil::mtl::strong_actor_ptr src_;
    nil::mtl::local_actor *dest_;
...
class allow_clause {
public:
    allow_clause(nil::mtl::scheduler::test_coordinator &sched) : sched_(sched), dest_(nullptr) {
        peek_ = [=] {
            if (dest_ != nullptr) {
                return try_extract<Ts...>(dest_) != nil::mtl::none;
            }
            return false;
        };
    }
    ...
    allow_clause &to(const nil::mtl::scoped_actor &whom) {
        dest_ = whom.ptr();
        return *this;
    }
...
public:
    disallow_clause() {
        check_ = [=] {
            ...
            if (res != nil::mtl::none) {
                BOOST_FAIL("received disallowed message: ");    // << MTL_ARG(*res));
            }
        };
    }
...
struct test_coordinator_fixture_fetch_helper {
    ...
    std::tuple<Ts...> operator()(nil::mtl::response_handle<Self, Interface, true> &from) const {
        std::tuple<Ts...> result;
        from.receive([&](Ts &... xs) { result = std::make_tuple(std::move(xs)...); },
                     [&](nil::mtl::error &err) { FAIL(from.self()->system().render(err)); });
        return result;
    }
};
...
struct test_coordinator_fixture_fetch_helper<T> {
    template<class Self, class Interface>
    T operator()(nil::mtl::response_handle<Self, Interface, true> &from) const {
        T result;
...
class test_coordinator_fixture {
    static inline nil::mtl::actor_system_config &config(nil::mtl::actor_system_config &cfg) {
...
public:
    ...
    explicit test_coordinator_fixture(Ts &&... xs) :
        ...
        sched.clock().time_per_unit.emplace(nil::mtl::atom("batch"), nil::mtl::timespan {1000});
    ...
    size_t advance_time(nil::mtl::timespan interval) {
    ...
    size_t run_until(BoolPredicate predicate) {
        ...
        for (;;) {
            ...
            while (consume_message()) {
                ++progress;
                if (predicate()) {
                    MTL_LOG_DEBUG("stop due to predicate:" << MTL_ARG(events));
                    return events;
                }
            }
            while (handle_io_event()) {
            ...
            if (progress == 0) {
                MTL_LOG_DEBUG("no activity left:" << MTL_ARG(events));
                return events;
            }
            events += progress;
        }
    ...
    typename std::conditional<sizeof...(Ts) == 0, T, std::tuple<T, Ts...>>::type request(Handle hdl, Us... args) {
        auto res_hdl = self->request(hdl, nil::mtl::infinite, std::move(args)...);
        run();
    ...
    template<class T = nil::mtl::scheduled_actor, class Handle = nil::mtl::actor>
    T &deref(const Handle &hdl) {
        auto ptr = nil::mtl::actor_cast<nil::mtl::abstract_actor *>(hdl);
        BOOST_REQUIRE(ptr != nullptr);
    ...
    void run_exhaustively() MTL_DEPRECATED_MSG("use run() instead");

    void run_exhaustively_until(std::function<bool()> f) MTL_DEPRECATED_MSG("use run_until() instead");

    void loop_after_next_enqueue() MTL_DEPRECATED_MSG("use run_after_next_ready_event() instead");

    nil::mtl::timespan credit_round_interval MTL_DEPRECATED;
};
...
template<class Config>
void test_coordinator_fixture<Config>::run_exhaustively_until(std::function<bool()> f) {
...
#define allow(types, fields)                                     \
    ([&] {                                                       \
        ...
        allow_clause<MTL_EXPAND(MTL_DSL_LIST types)> x {sched};  \
        x.fields;                                                \
        return x.eval();                                         \
    })()
...
#define disallow(types, fields)                                     \
    do {                                                            \
        ...
        disallow_clause<MTL_EXPAND(MTL_DSL_LIST types)> {}.fields;  \
    } while (false)
