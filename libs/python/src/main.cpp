...
namespace nil {
    namespace mtl {
        ...
        void register_class(atom_value *, pybind11::module &m, const std::string &name) {
            ...
            pybind11::class_<atom_value>(m, name.c_str()).def("__str__", to_string_fun).def("__repr__", repr_fun).def(
                    "__eq__", cmp);
        }
        ...
        namespace python {
            namespace {
                ...
                class binding {
                public:
                    binding(std::string py_name, bool builtin_type) : python_name_(std::move(py_name)),
                ...
                public:
                    ...
                    virtual pybind11::object to_object(const type_erased_tuple &xs, size_t pos) const = 0;
                };
                ...
                public:
                    ...
                    pybind11::object to_object(const type_erased_tuple &xs, size_t pos) const override {
                ...
                private:
                    template<class U>
                    static auto test(U *x) -> decltype(register_class(x, std::declval<pybind11::module &>(),
                ...
                typename std::enable_if<
                        !has_register_class<T>::value && has_to_string<T>::value>::type default_python_class_init(
                ...
                template<class T>
                typename std::enable_if<has_register_class<T>::value>::type default_python_class_init(
                        pybind11::module &m, const std::string &name) {
                ...
                public:
                    ...
                    friend void serialize(serializer &sink, absolute_receive_timeout &x, const unsigned int) {
                    ...
                    friend void serialize(deserializer &source, absolute_receive_timeout &x, const unsigned int) {
                ...
                void register_class(absolute_receive_timeout *, pybind11::module &m, const std::string &name) {
                    pybind11::class_<absolute_receive_timeout>(m, name.c_str()).def(pybind11::init<>()).def(
                            pybind11::init<int>());
                }
                ...
                public:
                    ...
                    using register_fun =
                    std::function< void (pybind11::module
                    &, const std::string&)>;
                    ...
                    std::string ipython_script() const {
                        ...
                        oss << "import IPython" << endl << "c = IPython.Config()" << endl
                            << "c.InteractiveShellApp.exec_lines = [" << endl << R"(""")" << full_pre_run << R"(""")"
                            << endl << "]" << endl << "c.PromptManager.in_template  = ' $: '" << endl
                            << "c.PromptManager.in2_template = ' -> '" << endl
                            << "c.PromptManager.out_template = ' >> '" << endl << "c.display_banner = True" << endl
                            << R"(c.TerminalInteractiveShell.banner1 = """)" << endl << banner << endl << R"(""")"
                            << endl << "IPython.start_ipython(config=c)" << endl;
                        return oss.str();
                    }
                ...
                private:
                    ...
                    void add_cpp(std::string py_name, std::string cpp_name,
                        ...
                        if (reg) {
                            register_funs_.push_back([=](pybind11::module &m) {
                                reg(m, py_name);
                            });
                        }
                ...
                void register_class(foo *, pybind11::module &m, const std::string &name) {
                    ...
                    pybind11::class_<foo>(m, name.c_str()).def(pybind11::init<>()).def(pybind11::init<int, int>()).def(
                            "__str__", str_fun).def("__repr__", str_fun).def_readwrite("x", &foo::x).def_readwrite("y",
                            &foo::y);
                }
...
#define MTL_MODULE_INIT_RET(unused)
                ...
                MTL_MODULE_INIT_RES caf_module_init() {
                    ...
                    // add classes
                    // add free functions
                    m.def("send", &py_send, "Sends a message to an actor").def("dequeue_message", &py_dequeue,
...
namespace {
    ...
    public:
        ...
        config() {
            ...
            opt_group{custom_options_, "python"}.add(py_file, "file,f", "Run script instead of interactive shell.");
        }
    };
