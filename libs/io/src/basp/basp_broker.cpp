    ...
    namespace mtl {
        namespace io {
            ...
            void basp_broker::on_exit() {
                // Wait until all pending messages of middleman_workers have been shipped.
            ...
            behavior basp_broker::make_behavior() {
                ...
                if (config().middleman_enable_automatic_connections) {
                    ...
                    if (res) {
                        ...
                        send(actor_cast<actor>(config_server), put_atom::value, "basp.default-connectivity-tcp",
                ...
                return behavior {
                    ...
                    [=](new_data_msg &msg) {
                        ...
                        if (next != ctx.cstate) {
                            auto rd_size = next == basp::await_payload ? ctx.hdr.payload_len : basp::header_size;
                    ...
                    [=](forward_atom, strong_actor_ptr &src, const std::vector<strong_actor_ptr> &fwd_stack,
                        ...
                        if (!instance.dispatch(context(), src, fwd_stack, dest->node(), dest->id(), 0, mid, msg) &&
                            mid.is_request()) {
                    ...
                    [=](forward_atom, const node_id &dest_node, atom_value dest_name,
                        ...
                        if (!instance.dispatch(context(), sender, cme->stages, dest_node,
                                               static_cast<uint64_t>(dest_name), basp::header::named_receiver_flag,
                                               cme->mid, msg)) {
                            detail::sync_request_bouncer srb {exit_reason::remote_link_unreachable};
                            srb(sender, cme->mid);
                        }
                    ...
                    [=](const connection_closed_msg &msg) {
                        ...
                        q.push(context(), msg_id, ctrl(),
                               make_mailbox_element(nullptr, make_message_id(), {}, delete_atom::value, msg.handle));
                    },
                    ...
                    [=](get_atom, const node_id &x) -> std::tuple<node_id, std::string, uint16_t> {
                        std::string addr;
                        uint16_t port = 0;
            ...
            resumable::resume_result basp_broker::resume(execution_unit *ctx, size_t mt) {
                ...
                auto guard = detail::make_scope_guard([=] { ctx->proxy_registry_ptr(nullptr); });
            ...
            strong_actor_ptr basp_broker::make_proxy(node_id nid, actor_id aid) {
                ...
                if (t_last_hop != nullptr && nid != *t_last_hop && instance.tbl().add_indirect(*t_last_hop, nid))
                ...
                actor_config cfg;
                auto res = make_actor<forwarding_actor_proxy, strong_actor_ptr>(aid, nid, &(system()), cfg, this);
            ...
            basp::instance::callee::buffer_type &basp_broker::get_buffer(connection_handle hdl) {
                return wr_buf(hdl);
            }
