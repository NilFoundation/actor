    ...
    namespace mtl {
        namespace io {
                            ...
                            request(broker_, infinite, connect_atom::value, std::move(ptr), port)
                                .then(
                                    [=](node_id &nid, strong_actor_ptr &addr, mpi_set &sigs) {
                                        ...
                                        auto res = make_message(std::move(nid), std::move(addr), std::move(sigs));
            ...
            middleman_actor_impl::put_res middleman_actor_impl::put(uint16_t port, strong_actor_ptr &whom,
                ...
                anon_send(broker_, publish_atom::value, std::move(ptr), actual_port, std::move(whom), std::move(sigs));
                return actual_port;
            }
            ...
            middleman_actor_impl::put_res middleman_actor_impl::put_udp(uint16_t port, strong_actor_ptr &whom,
                ...
                anon_send(broker_, publish_udp_atom::value, std::move(ptr), actual_port, std::move(whom),
                          std::move(sigs));
                return actual_port;
            }

            optional<middleman_actor_impl::endpoint_data &> middleman_actor_impl::cached_tcp(const endpoint &ep) {
            ...
            optional<middleman_actor_impl::endpoint_data &> middleman_actor_impl::cached_udp(const endpoint &ep) {
            ...
            optional<std::vector<response_promise> &> middleman_actor_impl::pending(const endpoint &ep) {
            ...
            expected<scribe_ptr> middleman_actor_impl::connect(const std::string &host, uint16_t port) {
                return system().middleman().backend().new_tcp_scribe(host, port);
            }

            expected<datagram_servant_ptr> middleman_actor_impl::contact(const std::string &host, uint16_t port) {
                return system().middleman().backend().new_remote_udp_endpoint(host, port);
            }

            expected<doorman_ptr> middleman_actor_impl::open(uint16_t port, const char *addr, bool reuse) {
                return system().middleman().backend().new_tcp_doorman(port, addr, reuse);
            }

            expected<datagram_servant_ptr> middleman_actor_impl::open_udp(uint16_t port, const char *addr, bool reuse) {
                return system().middleman().backend().new_local_udp_endpoint(port, addr, reuse);
            }
