    ...
    namespace actor {
        namespace io {
            namespace network {
                ...
                using interfaces_map = std::map<std::string, std::map<protocol::network, std::vector<std::string>>>;
                ...
                void for_each_address(bool get_ipv4, bool get_ipv6, F fun) {
                    ...
                    if (retval != NO_ERROR) {
                        std::cerr << "Call to GetAdaptersAddresses failed with error: " << retval << std::endl;
                        if (retval == ERROR_NO_DATA) {
                            std::cerr << "No addresses were found for the requested parameters" << std::endl;
                        } else {
                            void *msgbuf = nullptr;
                    ...
                    for (auto i = ifs.get(); i != nullptr; i = i->Next) {
                        for (auto j = i->FirstUnicastAddress; j != nullptr; j = j->Next) {
                            ...
                            if (family != AF_UNSPEC)
                                fun(i->AdapterName, family == AF_INET ? protocol::ipv4 : protocol::ipv6, false, buffer);
                        }
                    }
                }
                ...
                void interfaces::traverse(std::initializer_list<protocol::network> ps, consumer f) {
                ...
                std::vector<std::string> interfaces::list_addresses(std::initializer_list<protocol::network> procs,
                                                                    bool include_localhost) {
                    std::vector<std::string> result;
                    traverse_impl(procs, [&](const char *, protocol::network, bool lo, const char *addr) {
                        if (include_localhost || !lo)
                ...
                std::vector<std::string> interfaces::list_addresses(protocol::network proc, bool include_localhost) {
                    return list_addresses({proc}, include_localhost);
                }

                optional<std::pair<std::string, protocol::network>>
                    interfaces::native_address(const std::string &host, optional<protocol::network> preferred) {
                    addrinfo hint;
                    ...
                    for (auto i = addrs.get(); i != nullptr; i = i->ai_next) {
                        ...
                        if (family != AF_UNSPEC)
                            return std::make_pair(buffer, family == AF_INET ? protocol::ipv4 : protocol::ipv6);
                    }
                    return none;
                }

                std::vector<std::pair<std::string, protocol::network>>
                    interfaces::server_address(uint16_t port, const char *host, optional<protocol::network> preferred) {
                    ...
                    std::stable_sort(
                        ...
                        [](const addr_pair &lhs, const addr_pair &rhs) { return lhs.second > rhs.second; });
                    return results;
                }

                bool interfaces::get_endpoint(const std::string &host, uint16_t port, ip_endpoint &ep,
