    ...
    namespace mtl {
        namespace io {
            namespace network {
                ...
                test_multiplexer::scribe_data::scribe_data(shared_buffer_type input, shared_buffer_type output) :
                    vn_buf_ptr(std::move(input)), wr_buf_ptr(std::move(output)), vn_buf(*vn_buf_ptr),
                    wr_buf(*wr_buf_ptr), stopped_reading(false), passive_mode(false), ack_writes(false) {
                    // nop
                }

                test_multiplexer::doorman_data::doorman_data() : port(0), stopped_reading(false), passive_mode(false) {
                    // nop
                }
                ...
                test_multiplexer::datagram_data::datagram_data(shared_job_queue_type input,
                    ...
                    wr_buf_ptr(std::move(output)), vn_buf(*vn_buf_ptr), wr_buf(*wr_buf_ptr),
                    rd_buf(datagram_handle::from_int(0), receive_buffer_size), stopped_reading(false),
                    passive_mode(false), ack_writes(false), port(0), local_port(0), datagram_size(receive_buffer_size) {
                    // nop
                }
                ...
                datagram_servant_ptr test_multiplexer::new_datagram_servant(datagram_handle hdl, uint16_t port) {
                    ...
                    public:
                        ...
                        std::vector<datagram_handle> hdls() const override {
                            auto data = mpx_->data_for_hdl(hdl());
                            std::vector<datagram_handle> result(data->servants.begin(), data->servants.end());
                            return result;
                        }
                        void add_to_loop() override {
                            mpx_->passive_mode(hdl()) = false;
                ...
                bool test_multiplexer::is_known_port(uint16_t x) const {
                    auto pred1 = [&](const doorman_data_map::value_type &y) { return x == y.second.port; };
                    auto pred2 = [&](const datagram_data_map::value_type &y) { return x == y.second->port; };
                ...
                bool test_multiplexer::is_known_handle(accept_handle x) const {
                    auto pred = [&](const pending_doorman_map::value_type &y) { return x == y.second; };
                    return doorman_data_.count(x) > 0 || std::any_of(doormen_.begin(), doormen_.end(), pred);
                }
                ...
                std::shared_ptr<test_multiplexer::datagram_data> test_multiplexer::data_for_hdl(datagram_handle hdl) {
