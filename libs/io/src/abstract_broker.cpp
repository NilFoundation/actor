    ...
    namespace actor {
        namespace io {
            ...
            bool abstract_broker::cleanup(error &&reason, execution_unit *host) {
                ...
                close_all();
                ACTOR_ASSERT(doormen_.empty());
                ACTOR_ASSERT(scribes_.empty());
                ACTOR_ASSERT(datagram_servants_.empty());
            ...
            void abstract_broker::configure_read(connection_handle hdl, receive_policy::config cfg) {
                ACTOR_LOG_TRACE(ACTOR_ARG(hdl) << ACTOR_ARG(cfg));
                auto x = by_id(hdl);
                if (x)
            ...
            void abstract_broker::ack_writes(connection_handle hdl, bool enable) {
                ACTOR_LOG_TRACE(ACTOR_ARG(hdl) << ACTOR_ARG(enable));
                auto x = by_id(hdl);
                if (x)
                    x->ack_writes(enable);
            }
            ...
            byte_buffer &abstract_broker::wr_buf(connection_handle hdl) {
                ACTOR_ASSERT(hdl != invalid_connection_handle);
                auto x = by_id(hdl);
                if (!x) {
                    ACTOR_LOG_ERROR("tried to access wr_buf() of an unknown connection_handle:" << ACTOR_ARG(hdl));
            ...
            expected<std::pair<accept_handle, uint16_t>> abstract_broker::add_tcp_doorman(uint16_t port, const char *in,
                                                                                          bool reuse_addr) {
                ACTOR_LOG_TRACE(ACTOR_ARG(port) << ACTOR_ARG(in) << ACTOR_ARG(reuse_addr));
            ...
            void abstract_broker::add_datagram_servant(datagram_servant_ptr ptr) {
                ...
                ACTOR_ASSERT(ptr->parent() == nullptr);
                ptr->set_parent(this);
                auto hdls = ptr->hdls();
                launch_servant(ptr);
                for (auto &hdl : hdls)
            ...
            void abstract_broker::add_hdl_for_datagram_servant(datagram_servant_ptr ptr, datagram_handle hdl) {
                ACTOR_LOG_TRACE(ACTOR_ARG(ptr) << ACTOR_ARG(hdl));
                ACTOR_ASSERT(ptr != nullptr);
                ACTOR_ASSERT(ptr->parent() == this);
            ...
            void abstract_broker::move_datagram_servant(datagram_servant_ptr ptr) {
                ACTOR_LOG_TRACE(ACTOR_ARG(ptr));
                ACTOR_ASSERT(ptr != nullptr);
                ACTOR_ASSERT(ptr->parent() != nullptr && ptr->parent() != this);
                ptr->set_parent(this);
                ACTOR_ASSERT(ptr->parent() == this);
                auto hdls = ptr->hdls();
                for (auto &hdl : hdls)
            ...
            void abstract_broker::launch_servant(doorman_ptr &ptr) {
                ...
                // allows ACTOR to assign doorman to uninitialized brokers.
