...
public:
    ...
    /// result.
    template<class Handle = nil::mtl::actor>
    ...
    nil::mtl::io::middleman &mm;

    /// Reference to the middleman's event multiplexer.
    nil::mtl::io::network::test_multiplexer &mpx;
...
void exec_all_fixtures(Iterator first, Iterator last) {
    using fixture_ptr = nil::mtl::detail::decay_t<decltype(*first)>;
...
/// Base fixture for simulated network settings with any number of MTL nodes.
template<class PlanetType>
class test_network_fixture_base {
public:
    using planets_vector = std::vector<PlanetType *>;

    using connection_handle = nil::mtl::io::connection_handle;

    using accept_handle = nil::mtl::io::accept_handle;
