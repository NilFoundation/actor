        ...
        namespace io {
            namespace network {
                ...
                public:
                    ...
                    expected<scribe_ptr> new_tcp_scribe(const std::string &host, uint16_t port_hint) override;

                    doorman_ptr new_doorman(native_socket) override;

                    expected<doorman_ptr> new_tcp_doorman(uint16_t prt, const char *in, bool reuse_addr) override;
                ...
                public:
                    ...
                    datagram_servant_ptr new_datagram_servant(datagram_handle, const std::string &host, uint16_t port);
                    ...
                    void provide_datagram_servant(std::string host, uint16_t desired_port, datagram_handle hdl);
                    ...
                    void prepare_connection(accept_handle src, connection_handle hdl, test_multiplexer &peer,
                                            std::string host, uint16_t port, connection_handle peer_hdl);

                    /// Stores `hdl` as a pending endpoint for `src`.
                    void add_pending_endpoint(datagram_handle src, datagram_handle hdl);

                    using pending_connects_map = std::unordered_multimap<accept_handle, connection_handle>;
                    ...
                    using pending_scribes_map = std::map<std::pair<std::string, uint16_t>, connection_handle>;

                    using pending_doorman_map = std::unordered_map<uint16_t, accept_handle>;

                    using pending_local_datagram_endpoints_map = std::map<uint16_t, datagram_handle>;
                    ...
                    /// Appends `buf` to the virtual network buffer of `hdl`
                    /// and calls `read_data(hdl)` afterwards.
                    void virtual_send(datagram_handle src, datagram_handle ep, const buffer_type &);
                ...
                private:
                    ...
                    struct datagram_data {
                        ...
                        // Allows creating an entangled scribes where the input of this scribe is
                        // the output of another scribe and vice versa.
                        datagram_data(shared_job_queue_type input = std::make_shared<write_job_queue_type>(),
                    ...
                    using datagram_data_map = std::unordered_map<datagram_handle, std::shared_ptr<datagram_data>>;
