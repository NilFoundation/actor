        ...
        namespace io {
            namespace basp {
                ...
                struct header {
                    ...
                    header(message_type m_operation, uint8_t m_flags, uint32_t m_payload_len, uint64_t m_operation_data,
                           actor_id m_source_actor, actor_id m_dest_actor) :
                        operation(m_operation),
                        flags(m_flags), payload_len(m_payload_len), operation_data(m_operation_data),
                        source_actor(m_source_actor), dest_actor(m_dest_actor) {
                        // nop
                    }
                ...
                typename Inspector::result_type inspect(Inspector &f, header &hdr) {
                    uint8_t pad = 0;
                    return f(meta::type_name("header"), hdr.operation, meta::omittable(), pad, meta::omittable(), pad,
                             hdr.flags, hdr.payload_len, hdr.operation_data, hdr.source_actor, hdr.dest_actor);
                }
                ...
                constexpr size_t header_size = sizeof(actor_id) * 2 + sizeof(uint32_t) * 2 + sizeof(uint64_t);
