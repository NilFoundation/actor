...
namespace nil {
    namespace mtl {
        ...
        namespace io {
            ...
            using connection_handler = typed_actor<reacts_to<new_data_msg>, reacts_to<connection_closed_msg>>;
            ...
            /// @relates typed_broker
            using accept_handler = typed_actor<reacts_to<new_connection_msg>, reacts_to<acceptor_closed_msg>>;

            /// A typed broker mediates between actor systems and other
            /// components in the network.
            /// @extends local_actor
            template<class... Sigs>
            class typed_broker : public extend<abstract_broker, typed_broker<Sigs...>>::
            ...
            public:
                ...
                void initialize() override {
                    ...
                    MTL_LOG_DEBUG_IF(!bhvr,
                                     "make_behavior() did not return a behavior:" << MTL_ARG2("alive", this->alive()));
                    if (bhvr) {
                ...
                typename infer_handle_from_fun<F>::type fork(F fun, connection_handle hdl, Ts &&... xs) {
                    MTL_ASSERT(this->context() != nullptr);
                    auto sptr = this->take(hdl);
                    MTL_ASSERT(sptr->hdl() == hdl);
                    using impl = typename infer_handle_from_fun<F>::impl;
                    static_assert(std::is_convertible<typename impl::actor_hdl, connection_handler>::value,
                    ...
                    static constexpr bool spawnable = detail::spawnable<F, impl, decltype(hdl), Ts...>();
                    static_assert(spawnable, "cannot spawn function-based broker with given arguments");
                    detail::bool_token<spawnable> enabled;
                    auto res = this->system().spawn_functor(enabled, cfg, fun, hdl, std::forward<Ts>(xs)...);
                ...
                expected<std::pair<accept_handle, uint16_t>>
                    add_tcp_doorman(uint16_t port = 0, const char *in = nullptr, bool reuse_addr = false) {
