                ...
                public:
                    void handle_remote_message(execution_unit *ctx) {
                        ...
                        if (dst == nullptr && !mid.is_request()) {
                            ACTOR_LOG_INFO("drop asynchronous remote message: unknown destination");
                            return;
                        }
                        ...
                        if (dref.hdr_.operation == basp::message_type::routed_message) {
                            ...
                            if (auto err = source(src_node, dst_node)) {
                                ACTOR_LOG_ERROR("cannot read source and destination of remote message");
                                return;
                            }
                            ACTOR_ASSERT(dst_node == sys.node());
                            ...
                            ACTOR_ASSERT(dref.hdr_.operation == basp::message_type::direct_message);
                        ...
                        if (dst == nullptr) {
                            ...
                            ACTOR_LOG_INFO("drop remote request: unknown destination");
                        ...
                        if (auto err = source(stages, msg)) {
                            ACTOR_LOG_ERROR("cannot read stages and content of remote message");
                            return;
                        }
                            ...
                            switch (static_cast<uint64_t>(msg.get_as<atom_value>(0))) {
                                ...
                                case link_atom::uint_value(): {
                                    if (ptr != nullptr)
                                        ...
                                        ACTOR_LOG_WARNING("received link message with invalid target");
                                    return;
                                }
                                case unlink_atom::uint_value(): {
                                    if (ptr != nullptr)
                                        ...
                                        ACTOR_LOG_DEBUG("received unlink message with invalid target");
                                    return;
                                }
                            }
                        }
