...
#ifndef ACTOR_IO_NETWORK_ASIO_MULTIPLEXER_HPP
#define ACTOR_IO_NETWORK_ASIO_MULTIPLEXER_HPP
            ...
            namespace network {
                ...
                public:
                    ...
                    void write(const void *buf, size_t num_bytes) {
                        ACTOR_LOG_TRACE("num_bytes: " << num_bytes);
                ...
                private:
                    void read_loop(const manager_ptr &mgr) {
                        auto cb = [=](const boost::system::error_code &ec, size_t read_bytes) {
                            ACTOR_LOG_TRACE("nil::actor::io::network::stream", "read_loop$cb" << ACTOR_ARG(this));
                            if (!ec) {
                        ...
                        switch (rd_flag_) {
                            case receive_policy_flag::exactly:
                                ...
                                boost::asio::async_read(fd_, boost::asio::buffer(rd_buf_, rd_size_), cb);
                                break;
                    ...
                    void write_loop(const manager_ptr &mgr) {
                        ...
                        boost::asio::async_write(
                            fd_, boost::asio::buffer(wr_buf_), [=](const boost::system::error_code &ec, size_t nb) {
                                ACTOR_LOG_TRACE("nil::actor::io::network::stream", "write_loop$lambda", ACTOR_ARG(this));
                                static_cast<void>(nb);    // silence compiler warning
                                if (!ec) {
                                    ACTOR_LOGC(ACTOR_DEBUG, "nil::actor::io::network::stream", "write_loop$lambda",
                                             nb << " bytes sent");
                                    write_loop(mgr);
                                } else {
                                    ACTOR_LOGC(ACTOR_DEBUG, "nil::actor::io::network::stream", "write_loop$lambda",
                    ...
                    void collect_data(const manager_ptr &mgr, size_t collected_bytes) {
                        fd_.async_read_some(
                            ...
                            [=](const boost::system::error_code &ec, size_t nb) {
                                ACTOR_LOG_TRACE("nil::actor::io::network::stream", "collect_data$lambda" << ACTOR_ARG(this));
                                if (!ec) {
