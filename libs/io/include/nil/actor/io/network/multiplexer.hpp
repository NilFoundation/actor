            ...
            namespace network {
                ...
                public:
                    ...
                    virtual expected<doorman_ptr>
                        new_tcp_doorman(uint16_t port, const char *in = nullptr, bool reuse_addr = false) = 0;
                    ...
                    virtual datagram_servant_ptr new_datagram_servant_for_endpoint(native_socket fd,
                                                                                   const ip_endpoint &ep) = 0;

                    /// Create a new `datagram_servant` to contact a remote endpoint `host` and
                    /// `port`.
                    /// @warning Do not call from outside the multiplexer's event loop.
                    virtual expected<datagram_servant_ptr> new_remote_udp_endpoint(const std::string &host,
                                                                                   uint16_t port) = 0;
                    ...
                    virtual expected<datagram_servant_ptr>
                        new_local_udp_endpoint(uint16_t port, const char *in = nullptr, bool reuse_addr = false) = 0;
                    ...
                    /// Creates an instance using the networking backend compiled with ACTOR.
                    ...
                    /// Retrieves a pointer to the implementation or `nullptr` if ACTOR was
