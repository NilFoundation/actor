        ...
        namespace network {
            ...
            public:
                ...
                bool handle_read_event(endpoint_manager &) override {
                    MTL_LOG_TRACE(MTL_ARG(this->handle_.id));
                    for (size_t reads = 0; reads < this->max_consecutive_reads_; ++reads) {
                        ...
                        if (auto res = get_if<std::pair<size_t, ip_endpoint>>(&ret)) {
                            ...
                            MTL_LOG_DEBUG("received " << num_bytes << " bytes");
                ...
                bool handle_write_event(endpoint_manager &manager) override {
                    MTL_LOG_TRACE(MTL_ARG2("handle", this->handle_.id) << MTL_ARG2("queue-size", packet_queue_.size()));
                ...
                error write_some() {
                    ...
                    while (!packet_queue_.empty()) {
                        ...
                        if (auto num_bytes = get_if<size_t>(&write_ret)) {
                            ...
                            MTL_LOG_WARNING_IF(*num_bytes < packet.size, "packet was not sent completely");
                            recycle();
                        } else {
                            auto err = get<sec>(write_ret);
                            if (err != sec::unavailable_or_would_block) {
                                MTL_LOG_ERROR("write failed" << MTL_ARG(err));
                                this->next_layer_.handle_error(err);
                            }
                            return err;
                        }
