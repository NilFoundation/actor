...
namespace nil {
    namespace mtl {
        namespace network {
            ...
            /// streams.
            struct MTL_NET_EXPORT stream_socket : network_socket {
            ...
            expected<std::pair<stream_socket, stream_socket>> MTL_NET_EXPORT make_stream_socket_pair();

            /// Enables or disables keepalive on `x`.
            /// @relates network_socket
            error MTL_NET_EXPORT keepalive(stream_socket x, bool new_value);

            /// Enables or disables Nagle's algorithm on `x`.
            /// @relates stream_socket
            error MTL_NET_EXPORT nodelay(stream_socket x, bool new_value);
            ...
            variant<size_t, sec> MTL_NET_EXPORT read(stream_socket x, span<byte> buf);
            ...
            /// @relates stream_socket
            /// @post either the result is a `sec` or a positive (non-zero) integer
            variant<size_t, sec> MTL_NET_EXPORT write(stream_socket x, span<const byte> buf);
            ...
            /// @post either the result is a `sec` or a positive (non-zero) integer
            /// @pre `bufs.size() < 10`
            variant<size_t, sec> MTL_NET_EXPORT write(stream_socket x, std::initializer_list<span<const byte>> bufs);

            /// Converts the result from I/O operation on a ::stream_socket to either an
            /// error code or a non-zero positive integer.
            /// @relates stream_socket
            variant<size_t, sec> MTL_NET_EXPORT check_stream_socket_io_res(std::make_signed<size_t>::type res);
        }    // namespace network
    }        // namespace mtl
}    // namespace nil
